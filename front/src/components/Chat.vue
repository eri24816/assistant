...
const processMessage = (message: any, toolCallMessages: Map<string, Message>) => {
    if (message.role == 'assistant') {
        ...
        toolCallMessages.set(message.call_id, uiMessage)
    }
    else if (message.type == 'function_call_output') {
        let uiMessage: Message = toolCallMessages.get(message.call_id)!
        uiMessage.collapsableContent = message.output
    }
}
...
const processMessageChunk = (chunk: any, accumulatingMessage: Message) => {
    ...
    if (chunk.type === 'ai') {
        ...
        currentRole = 'ai'
    } else if (chunk.type === 'tool_call' || chunk.type === 'tool_call_delta' || chunk.type === 'tool_result') {
        currentRole = 'system'
    }else{
